...
for (i1 in c("T1", "T2")) {
  ...
  for (i2 in 1:length(pss.norm.ls)) {
    ...
    for (i3 in used.dist) {
        ...
        res.names <- paste0(names(pss.norm.ls)[i2], "_", i3, "_", i1)  
...
```{r , message=FALSE, warning=FALSE, echo=FALSE, results='asis'}
res.maas.long[order(res.maas.long$qval), ] %>% 
      print(kable(.[1:50, ]))
```
\
\
...
for (i1.ps in names(pss.norm.ls)) {
     ...
     ps.dist <- pss.norm.ls[[i1.ps]] %>% 
        ...
        for (i2.dist in used.dist) {
        
        res.names <- paste0(i1.ps, "_", i2.dist)  
      
        dist.inst <- distance(ps.dist, method = i2.dist, type = "samples")
